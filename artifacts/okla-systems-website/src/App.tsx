        ...
        <section className="section section-dark">
          <div className="site-container">
            ...
            <div className="partners-grid">
              {[
                { name: 'HP', logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a1/HP_logo_2012.svg/200px-HP_logo_2012.svg.png' },
                { name: 'Dell', logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/Dell_logo_2016.svg/200px-Dell_logo_2016.svg.png' },
                { name: 'AXIZ', logo: 'https://www.axiz.com/wp-content/uploads/2020/06/axiz-logo.png' },
                { name: 'Westcon-Comstor', logo: 'https://www.westcon.com/wp-content/uploads/2021/03/Westcon-Comstor-Logo.png' },
                { name: 'Xerox', logo: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/Xerox_logo_2019.svg/200px-Xerox_logo_2019.svg.png' },
              ].map((partner, index) => <Reveal key={index} delay={`delay-${index + 1}`}><div className="partner-card" data-testid={`partner-${index}`}><img src={partner.logo} alt={partner.name} className="partner-logo" /></div></Reveal>)}
            </div>
          </div>
        </section>
    ...
    <main>
      ...
      <section className="section">
        <div className="site-container contact-layout">
          ...
          <Reveal delay="delay-2"><div className="contact-form-wrap">{submitted ? <div className="success-state" data-testid="status-form-success"><div><div className="success-icon"><Check size={25} /></div><h2>Enquiry received.</h2><p>Thank you, {values.name.split(' ')[0] || 'there'}. Our team will review the brief and be in touch soon.</p><a className="btn btn-orange" href="mailto:info@oklasystems.co.za" data-testid="link-success-email">Open your email client <Send size={14} /></a></div></div> : <><div className="form-header"><h2>Start with the brief.</h2></div><form action="https://formspree.io/f/xqazpvla" method="POST" noValidate data-testid="form-contact"><div className="form-grid"><div className="field"><label htmlFor="contact-name">Full name</label><input id="contact-name" name="name" value={values.name} onChange={update('name')} placeholder="Your name" data-testid="input-contact-name" />{errors.name && <span className="error-text">{errors.name}</span>}</div><div className="field"><label htmlFor="contact-email">Email address</label><input id="contact-email" type="email" name="email" value={values.email} onChange={update('email')} placeholder="you@company.co.za" data-testid="input-contact-email" />{errors.email && <span className="error-text">{errors.email}</span>}</div><div className="field"><label htmlFor="contact-phone">Phone</label><input id="contact-phone" type="tel" name="phone" value={values.phone} onChange={update('phone')} placeholder="081 289 7112" data-testid="input-contact-phone" />{errors.phone && <span className="error-text">{errors.phone}</span>}</div><div className="field"><label htmlFor="contact-subject">What can we help with?</label><input id="contact-subject" name="subject" value={values.subject} onChange={update('subject')} placeholder="A project, a system, a challenge" data-testid="input-contact-subject" /></div><div className="field full"><label htmlFor="contact-message-long">Message</label><textarea id="contact-message-long" name="message" value={values.message} onChange={update('message')} placeholder="Give us the useful context..." data-testid="input-contact-message" />{errors.message && <span className="error-text">{errors.message}</span>}</div></div><button type="submit" className="btn btn-orange" data-testid="button-submit-form">Send enquiry <Send size={14} /></button></form></>}</div></Reveal>
        </div>
