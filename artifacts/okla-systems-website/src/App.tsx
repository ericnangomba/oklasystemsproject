...
function Contact() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(next).length === 0) {
      const subject = encodeURIComponent(values.subject || 'Website Enquiry');
      const body = encodeURIComponent(`Name: ${values.name}\nEmail: ${values.email}\nPhone: ${values.phone}\n\nMessage:\n${values.message}`);
      window.location.href = `mailto:info@oklasystems.co.za?subject=${subject}&body=${body}`;
      setSubmitted(true);
    }
  };
  return (
